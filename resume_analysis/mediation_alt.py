...
if __name__ == "__main__":
    ...
    for RESUME_ID in RESUME_IDS:
        ...
        results = mediation_analysis(
            df, "variant", "decision", mediator_columns
        )
        ...
        print(
            f"  95% CI: [{results['direct_ci'][0]:.4f}, {results['direct_ci'][1]:.4f}]"
        )
        ...
        print(
            f"  95% CI: [{results['indirect_ci'][0]:.4f}, {results['indirect_ci'][1]:.4f}]"
        )

        is_sig = (
            "Yes"
            if (results["indirect_ci"][0] > 0 or results["indirect_ci"][1] < 0)
            else "No"
        )
        ...
        prop_med = (
            abs(results["indirect_effect"] / total * 100) if total != 0 else 0
        )
        print(f"\nAdditional Information:")
        print(f"  Total Effect: {total:.4f}")
        print(f"  Proportion Mediated: {prop_med:.1f}%")
        print("=" * 60)
        mean_prod_mediated.append(prop_med)
    ...
    for resume_id, res in results_all.items():
        sig = (
            "✓"
            if (res["indirect_ci"][0] > 0 or res["indirect_ci"][1] < 0)
            else "✗"
        )
        print(
            f"Resume {resume_id}: Indirect={res['indirect_effect']:.4f} [{sig}], Direct={res['direct_effect']:.4f}"
        )
    print(f"Mean Proportion Mediated: {np.mean(mean_prod_mediated):.1f}%")
    print(f"Std Proportion Mediated: {np.std(mean_prod_mediated):.1f}%")
