...
def normalize_target_name(raw_target: Any) -> str:
    """Normalize target name to lowercase string, handling tuples and other formats."""
    if isinstance(raw_target, tuple):
        # Extract first element if it's a tuple
        raw_target = raw_target[0] if raw_target else 'unknown'
    # Convert to string and lowercase
    return str(raw_target).lower().strip()
...
def analyze_onpolicy_base(input_dir: Path, output_dir: Path, targets: Optional[List[str]] = None, max_scenarios: Optional[int] = None, include_embedding_analyses: bool = True) -> None:
    ...
    for sdir in tqdm(scen_dirs, desc=f"{model_name}: scenarios"):
        ...
        for res_path in tqdm(files, desc=f"{sdir.name}: targets", leave=False):
            ...
            target_name = normalize_target_name(m.group(1) if m else "unknown")
            # Skip self_preservation
            if target_name == "self_preservation":
                continue
    ...
    for tgt, sdf in df.groupby(group_cols):
        # Normalize target name (handles any edge cases)
        tgt = normalize_target_name(tgt)
            ...
            for sdir in off_scen:
                ...
                for exp in (data_off or {}).get('experiments', []):
                    ...
                    for dr in exp.get('disruption_results', []) or []:
                        tgt_name = normalize_target_name(dr.get('strategy') or 'unknown')
                        # Skip self_preservation
                        if tgt_name == "self_preservation":
                            continue
...
def _plot_grouped_bars_by_target(df: pd.DataFrame, value_col: str, se_col: str, models: List[str], out_path: Path, title: str, y_label: str, ylim: Optional[Tuple[float, float]] = None) -> None:
    ...
    _FS_XTICK = 16 * 1.25 * 1.15
    _FS_YTICK = 16 * 1.6
    _FS_YLAB = 18 * 1.5
    _FS_TITLE = 27
    _FS_LEG = 16 * 1.15 * 1.15
    _FS_LEG_TITLE = 16
