...
parser.add_argument('-spm', '--skip-per-model', action='store_true', help='Skip per-model analysis if CSV files exist, only generate cross-model plots (much faster)')
...
def analyze_fighting_back_results(results: List[Dict], output_dir: Path, importance_scores: Dict[int, float], chunk_categories: Dict[int, List[str]], chunk_positions: Dict[int, float]) -> None:
    ...
    if not df_iterations.empty:
        ...
        axes[1, 1].plot(persistence_by_iter['iteration'].values, persistence_by_iter['new_reasoning_found'].values, 
                       marker='o', label='New Reasoning Found')
        axes[1, 1].plot(persistence_by_iter['iteration'].values, persistence_by_iter['contains_blackmail'].values, 
...
def analyze_reason_substitution_results(results: List[Dict], output_dir: Path, importance_scores: Dict[int, float], chunk_categories: Dict[int, List[str]], chunk_positions: Dict[int, float]) -> None:
    ...
    if not df_substitution.empty and 'position' in df_substitution.columns and 'severity_change' in df_substitution.columns:
        valid_change_data = df_substitution.dropna(subset=['severity_change'])
        if not valid_change_data.empty:
            axes[1, 1].scatter(valid_change_data['position'].values, valid_change_data['severity_change'].values, alpha=0.6)
            axes[1, 1].axhline(y=0, color='red', linestyle='--', alpha=0.5, label='No Change')
            
            # Add trendline
            z = np.polyfit(valid_change_data['position'].values, valid_change_data['severity_change'].values, 1)
            p = np.poly1d(z)
            axes[1, 1].plot(valid_change_data['position'].values, p(valid_change_data['position'].values), "r--", alpha=0.8)
...
def analyze_position_based_analysis(experiment_data: List[Dict], rollout_data: List[Dict], output_dir: Path) -> None:
    ...
    if not df_rollouts.empty and 'position' in df_rollouts.columns and 'fighting_back_score_semantic' in df_rollouts.columns:
        ...
        axes[0, 0].scatter(position_fighting_stats['position'].values, position_fighting_stats['mean'].values, alpha=0.6)
        
        # Add trendline
        z = np.polyfit(position_fighting_stats['position'].values, position_fighting_stats['mean'].values, 1)
        p = np.poly1d(z)
        axes[0, 0].plot(position_fighting_stats['position'].values, p(position_fighting_stats['position'].values), "r--", alpha=0.8)
    ...
    if not df_experiments.empty and 'position' in df_experiments.columns and 'abandonment_rate_semantic' in df_experiments.columns:
        ...
        axes[0, 1].scatter(position_abandon_stats['position'].values, position_abandon_stats['mean'].values, alpha=0.6, color='orange')
        
        # Add trendline
        z = np.polyfit(position_abandon_stats['position'].values, position_abandon_stats['mean'].values, 1)
        p = np.poly1d(z)
        axes[0, 1].plot(position_abandon_stats['position'].values, p(position_abandon_stats['position'].values), "r--", alpha=0.8)
...
def generate_combined_semantic_entropy_grouped_bars(model_names: List[str], output_dir: Path) -> None:
    """Create grouped bar charts of semantic entropy means per category across models.

    For each model, we traverse scenario directories under
    blackmail_rollouts/{model}/temperature_0.7_top_p_0.95/yes_base_solution,
    read semantic_entropy and function tags from chunks_labeled.json, expand by
    category (omitting globally excluded categories), and aggregate mean ± SE.

    Styling and layout are aligned with the Counterfactual++ importance plot.
    """
    print("Creating cross-model grouped-bar semantic entropy plots...")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for model in model_names:
        base_dir = Path(f"blackmail_rollouts/{model}/temperature_0.7_top_p_0.95/yes_base_solution")
        if not base_dir.exists():
            print(f"  Skipping {model}: base dir not found {base_dir}")
            continue

        rows: List[Dict] = []
        scenario_dirs = sorted([d for d in base_dir.iterdir() if d.is_dir() and d.name.startswith('scenario_')])
        for scen_dir in scenario_dirs:
            ...
            try:
                ...
                continue
            for ch in chunks_data or []:
                try:
                    ent = ch.get('semantic_entropy', None)
                    if ent is None:
                        continue
                    ent_val = float(ent)
                except Exception:
                    continue
                cats = ch.get('function_tags') or ch.get('categories') or []
                if isinstance(cats, str):
                    cats = [cats]
                for c in cats:
                    if not c:
                    ...
                    if c_l in exclude_cats_lower:
                        continue
                    rows.append({'category': _format_category_label_kl(c), 'value': ent_val})

        df = pd.DataFrame(rows)
        if df.empty:
            print(f"  Skipping {model}: no semantic entropy data found")
            continue

        grouped = df.groupby('category')['value'].agg(['mean', 'std', 'count']).reset_index()
        grouped['se'] = grouped.apply(
            lambda r: (r['std'] / np.sqrt(r['count'])) if r['count'] > 1 and not np.isnan(r['std']) else 0.0,
            axis=1
        )
        per_model_stats[model] = grouped
        models_with_data.append(model)

    if not models_with_data:
        print("No models with semantic entropy data found. Skipping plot.")
        return
    ...
    if not combined_rows:
        print("No aggregated semantic entropy rows to plot.")
        return
    ...
    present_map = {_norm_cat(c): c for c in categories}
    categories = [present_map[c] for c in [_norm_cat(d) for d in desired_order] if c in present_map] or categories

    # Pivot values/errors with enforced order
    pivot_vals = pd.pivot_table(df_combined, index='category', columns='model', values='mean', aggfunc='mean').reindex(index=categories, columns=models_with_data)
    pivot_errs = pd.pivot_table(df_combined, index='category', columns='model', values='se', aggfunc='mean').reindex(index=categories, columns=models_with_data)

    # Display labels aligned to enforced order
    category_labels = [_format_category_label_kl(c) for c in categories]

    # Plot grouped bars for ALL categories, matching CL++ styling
    num_models = len(models_with_data)
    x = np.arange(len(categories))
    width = max(0.1, 0.8 / max(1, num_models))
    colors = plt.cm.tab10.colors if num_models <= 10 else plt.cm.tab20.colors

    _FIG_W = max(16, 2.8 * len(categories))
    plt.figure(figsize=(_FIG_W, 7))
    for i, model in enumerate(models_with_data):
        vals = pivot_vals[model].values.astype(float)
        errs = pivot_errs[model].values.astype(float) if pivot_errs is not None and model in pivot_errs.columns else None
    ...
    wrapped_labels = [_wrap_two_words_cfpp(c) for c in category_labels]
    _FS_XTICK = 16 * 1.5
    _FS_YLAB = 18 * 1.5
    _FS_TITLE = 27
    _FS_LEG = 16 * 1.15
    _FS_LEG_TITLE = 16
    plt.xticks(x, wrapped_labels, rotation=0, ha='center', fontsize=_FS_XTICK)
    plt.ylabel('Semantic entropy', fontsize=_FS_YLAB)
    plt.title('Semantic entropy by category across models', fontsize=_FS_TITLE, pad=12)
    plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.50), ncol=min(len(models_with_data), 4), framealpha=0.9, fontsize=_FS_LEG, title="Models", title_fontsize=_FS_LEG_TITLE)
    ax = plt.gca()
    for side in ['top', 'right']:
        ax.spines[side].set_visible(False)
    ...
    plt.subplots_adjust(bottom=0.50)
    plt.tight_layout()
    save_name = 'semantic_entropy_by_category_across_models.png'
    plt.savefig(output_dir / save_name, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved grouped bars: {output_dir / save_name}")

def generate_cross_model_sentence_resilience_histograms(model_names: List[str], output_dir: Path) -> None:
    """Create a 2x2 figure with one histogram per model of per-sentence resilience.

    Resilience is taken from 'avg_fighting_back_score_semantic' in
    analysis/{model}/determination/fighting_back_experiments.csv.
    If unavailable, falls back to 'persistence_rate_semantic'.

    The figure is saved under the cross-model determination folder.
    """
    print("Creating cross-model sentence resilience histograms...")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Prefer avg_fighting_back_score_semantic; fallback to persistence_rate_semantic per model
    model_to_values: Dict[str, np.ndarray] = {}
    for model in model_names[:4]:  # limit to first four models for a 2x2 grid
        csv_path = Path(f"analysis/{model}/determination/fighting_back_experiments.csv")
        if not csv_path.exists():
            print(f"  Skipping {model}: {csv_path} not found")
            continue
        ...
        vals = None
        if 'avg_fighting_back_score_semantic' in df.columns:
            vals = df['avg_fighting_back_score_semantic'].dropna().astype(float).values
        if (vals is None or len(vals) == 0) and 'persistence_rate_semantic' in df.columns:
            vals = df['persistence_rate_semantic'].dropna().astype(float).values

        if vals is None or len(vals) == 0:
            print(f"  Skipping {model}: no resilience-like columns present")
            continue

        model_to_values[model] = vals

    if not model_to_values:
        print("No resilience data found across models; skipping histograms.")
        return

    # Compute global binning for comparability
    all_vals = np.concatenate(list(model_to_values.values()))
    ...
    # Helper to standardize model legend names
    def _legend_model_name(m: str) -> str:
        if m == 'deepseek-r1-0528':
            return 'deepseek-r1-0528'
        if m == 'qwen3-235b-a22b':
    ...
    # Keep original order limited to first four entries with data
    selected_models = [m for m in model_names if m in model_to_values][:4]
    for i in range(4):
        ax = axes[i]
        if i < len(selected_models):
            m = selected_models[i]
            vals = model_to_values[m]
            ax.hist(vals, bins=bins, range=(vmin, vmax), alpha=0.8, edgecolor='black')
            ax.set_title(f"{_legend_model_name(m)}")
            ax.set_xlabel('Average resilience score')
    ...
    plt.tight_layout()
    save_path = output_dir / 'cross_model_sentence_resilience_histograms.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved histograms: {save_path}")

def generate_cross_model_kl_vs_resilience_scatter(model_names: List[str], output_dir: Path) -> None:
    """Create a 2x2 panel plot with mean KL divergence (y-axis) vs mean resilience score (x-axis).
    
    Each panel shows one model, with categories as colored points with error bars.
    Saves to analysis/determination/cross_model_kl_vs_resilience_scatter.png.
    """
    print("Creating cross-model KL vs resilience scatter (2x2 panels)...")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    cache_file = output_dir / 'kl_vs_resilience_cache.json'
    if cache_file.exists():
        print("  Loading cached data...")
        try:
            with open(cache_file, 'r', encoding='utf-8') as f:
                cache_data = json.load(f)
            df_stats = pd.DataFrame(cache_data['stats'])
    ...
    if df_stats is None:
        # Collect all data points across models
        all_data = []
        
        for model in model_names:
            kl_csv_path = Path(f"analysis/{model}/determination/fighting_back_kl_per_sentence.csv")
            resilience_csv_path = Path(f"analysis/{model}/determination/fighting_back_experiments.csv")
            
            if not kl_csv_path.exists() or not resilience_csv_path.exists():
                print(f"  Skipping {model}: missing KL or resilience CSV")
                continue
        
            try:
                df_kl = pd.read_csv(kl_csv_path)
                df_resilience = pd.read_csv(resilience_csv_path)
            except Exception as e:
                print(f"  Skipping {model}: failed reading CSV: {e}")
                continue
            
            if df_kl.empty or df_resilience.empty:
                print(f"  Skipping {model}: empty dataframes")
                continue
            
            # Check for required columns
            if 'kl_after_vs_before' not in df_kl.columns or 'primary_tag' not in df_kl.columns:
                print(f"  Skipping {model}: missing required KL columns")
                continue
            
            if 'scenario_id' not in df_resilience.columns or 'chunk_idx' not in df_resilience.columns:
                print(f"  Skipping {model}: missing required resilience columns")
                continue
            
            # Get resilience metric (prefer avg_fighting_back_score_semantic)
            resilience_col = None
            if 'avg_fighting_back_score_semantic' in df_resilience.columns:
                resilience_col = 'avg_fighting_back_score_semantic'
            elif 'persistence_rate_semantic' in df_resilience.columns:
                resilience_col = 'persistence_rate_semantic'
            
            if resilience_col is None:
                print(f"  Skipping {model}: no resilience metric found")
                continue
            
            # Merge KL and resilience data on (scenario_id, chunk_idx)
            df_kl_subset = df_kl[['scenario_id', 'chunk_idx', 'kl_after_vs_before', 'primary_tag', 'n_after_rollouts']].copy()
            df_resilience_subset = df_resilience[['scenario_id', 'chunk_idx', resilience_col, 'total_rollouts']].copy()
            
            merged = pd.merge(
                df_kl_subset,
                df_resilience_subset,
                on=['scenario_id', 'chunk_idx'],
                how='inner'
            )
            
            if merged.empty:
                print(f"  Skipping {model}: no matching data after merge")
                continue
            
            # Drop rows with NaN values
            merged = merged.dropna(subset=['kl_after_vs_before', resilience_col, 'primary_tag'])
            
            # Exclude globally filtered categories
            merged = merged[~merged['primary_tag'].fillna('').str.lower().isin(exclude_cats_lower)]
            
            if merged.empty:
                print(f"  Skipping {model}: no data after filtering")
                continue
            
            # Add model info and format categories
            merged['model'] = model
            merged['category'] = merged['primary_tag'].apply(_format_category_label)
            merged['resilience'] = merged[resilience_col]
            merged['kl'] = merged['kl_after_vs_before']
            
            all_data.append(merged[['model', 'category', 'resilience', 'kl', 'n_after_rollouts', 'total_rollouts']])
        
        if not all_data:
            print("No data available for KL vs resilience scatter plot")
            return
        ...
        # Compute means and standard errors for each model-category combination
        stats_list = []
        for model in model_names:
            if model not in df_combined['model'].values:
                continue
            
            model_data = df_combined[df_combined['model'] == model]
            
            for category in sorted(model_data['category'].unique()):
                cat_data = model_data[model_data['category'] == category]
                if cat_data.empty:
                    continue
                
                # Resilience stats
                # Each row in cat_data is an independent measurement (scenario-chunk combination)
                resilience_mean = float(cat_data['resilience'].mean())
                resilience_std = float(cat_data['resilience'].std())
                n_resilience = len(cat_data)  # Number of independent measurements
                resilience_se = (resilience_std / np.sqrt(n_resilience)) if n_resilience > 1 and not np.isnan(resilience_std) else 0.0
                
                # KL stats
                # Each row in cat_data is an independent measurement (scenario-chunk combination)
                kl_mean = float(cat_data['kl'].mean())
                kl_std = float(cat_data['kl'].std())
                n_kl = len(cat_data)  # Number of independent measurements
                kl_se = (kl_std / np.sqrt(n_kl)) if n_kl > 1 and not np.isnan(kl_std) else 0.0
                
                stats_list.append({
                    'model': model,
                    'category': category,
                    'resilience_mean': resilience_mean,
                    'resilience_se': resilience_se,
                    'kl_mean': kl_mean,
                    'kl_se': kl_se,
                    'n_sentences': len(cat_data)
                })
    ...
    if n_cats <= 10:
        palette_colors = sns.color_palette('tab10', n_colors=n_cats)
    else:
        palette_colors = sns.color_palette('tab20', n_colors=n_cats)
    
    cat_to_color = {cat: palette_colors[i % len(palette_colors)] for i, cat in enumerate(categories)}
    
    # Model marker mapping
    model_markers = {
        'qwq-32b': 'o',
        'qwen3-235b-a22b': '^',
        'llama-3_1-nemotron-ultra-253b-v1': 's',
        'deepseek-r1-0528': 'D'
    }
    ...
    for _, row in df_stats.iterrows():
        ...
        # Only show error bars if SE > 0
        xerr_val = row['resilience_se'] if row['resilience_se'] > 0 else None
        yerr_val = row['kl_se'] if row['kl_se'] > 0 else None
        
        # Plot with enhanced visibility: larger markers, thicker error bars, less transparency
        ax.errorbar(
            row['resilience_mean'],
            row['kl_mean'],
            xerr=xerr_val,
            ...
            markeredgewidth=2.0,  # Increased from 1.5 for better contrast
            ecolor=color,
            elinewidth=2.0,  # Increased from 1.0 for visibility
            capsize=5,  # Increased from 3
            capthick=2.0,  # Increased from 1.0
            alpha=0.85,  # Increased from 0.7 for better visibility
            linestyle='none',
            zorder=3
        )
    
    ax.set_xlabel('Average resilience score', fontsize=16)
    ax.set_ylabel('Mean KL divergence', fontsize=16)
    ax.set_title('Counterfactual++ importance vs resilience scores\nby category across models', fontsize=18, pad=20)
    ...
    cat_handles = [
        Line2D([0], [0], marker='o', linestyle='None',
               markerfacecolor=cat_to_color[cat], markeredgecolor='white',
               markersize=11, markeredgewidth=1.5, label=cat)
        for cat in categories
    ]
    
    # Model legend (shapes) - use a neutral color
    model_handles = []
    for model in model_names[:4]:  # Limit to 4 models
        if model in df_stats['model'].values:
            marker = model_markers.get(model, 'o')
    ...
    # Position legends - categories on right, models on bottom
    legend1 = ax.legend(handles=cat_handles, title='Category',
    ...
    ax.legend(handles=model_handles, title='Model',
             ...
             ncol=4, frameon=True, framealpha=0.95, edgecolor='gray',
             fontsize=14, title_fontsize=14)
    
    plt.tight_layout()
    plt.subplots_adjust(right=0.80, bottom=0.15)
    
    save_path = output_dir / 'cross_model_kl_vs_resilience_scatter.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved scatter plot: {save_path}")
    
    # Create alternative visualization: Bubble chart
    # Bubble size represents confidence (inverse of combined uncertainty)
    print("  Creating bubble chart variant...")
    fig, ax = plt.subplots(1, 1, figsize=(14, 10))
    
    # Calculate bubble sizes based on inverse of total uncertainty
    # Smaller uncertainty = larger bubble = more confident
    for _, row in df_stats.iterrows():
        model = row['model']
        category = row['category']
        marker = model_markers.get(model, 'o')
        ...
        # Combined uncertainty (normalize to get reasonable bubble sizes)
        total_se = row['resilience_se'] + row['kl_se']
        # Inverse for size (add small constant to avoid division by zero)
        confidence = 1.0 / (total_se + 0.0001)
        # Scale to reasonable bubble size (100-1000 range)
        bubble_size = 100 + (confidence / df_stats.apply(lambda r: 1.0/(r['resilience_se'] + r['kl_se'] + 0.0001), axis=1).max()) * 900
        
        # Plot with bubble size representing confidence
        ax.scatter(
            row['resilience_mean'],
            row['kl_mean'],
            s=bubble_size,
    ...
    ax.set_xlabel('Average resilience score', fontsize=18)
    ax.set_ylabel('Counterfactual++ importance', fontsize=18)
    ax.set_title('Counterfactual++ importance vs resilience\n(bubble size = measurement confidence)', 
    ...
    cat_handles = [
        Line2D([0], [0], marker='o', linestyle='None',
               markerfacecolor=cat_to_color[cat], markeredgecolor='white',
               markersize=11, markeredgewidth=1.5, label=cat, alpha=0.8)
        for cat in categories
    ]
        ...
        if model in df_stats['model'].values:
            ...
            model_handles.append(
                Line2D([0], [0], marker=marker, linestyle='None',
                       markerfacecolor='gray', markeredgecolor='white',
                       markersize=11, markeredgewidth=1.5, 
                       label=_legend_model_name(model), alpha=0.8)
            )
    
    # Size legend (add reference bubbles)
    size_handles = [
        Line2D([0], [0], marker='o', linestyle='None',
               markerfacecolor='gray', markeredgecolor='white',
               markersize=s, markeredgewidth=1.5, 
               label=label, alpha=0.6)
        for s, label in [(6, 'Low confidence'), (10, 'Medium'), (14, 'High confidence')]
    ]
    ...
    ax.legend(handles=size_handles, title='Confidence',
             loc='upper center', bbox_to_anchor=(0.5, -0.12),
             ncol=3, frameon=True, framealpha=0.95, edgecolor='gray',
             fontsize=13, title_fontsize=14)
    
    plt.tight_layout()
    plt.subplots_adjust(right=0.78, bottom=0.15)
    
    save_path_bubble = output_dir / 'cross_model_kl_vs_resilience_bubble.png'
    plt.savefig(save_path_bubble, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved bubble chart: {save_path_bubble}")
    
    # Create alternative visualization: Slope/Arrow chart
    # Show arrows from origin pointing to (resilience, KL) for each category-model combination
    print("  Creating arrow/vector chart...")
    fig, ax = plt.subplots(1, 1, figsize=(14, 10))
    ...
    for _, row in df_stats.iterrows():
        ...
        # Draw arrow from origin (or from a reference point)
        # We'll use a reference point at (0, 0) or mean of all points
        x_start = 0
        y_start = 0
        
        # Draw line from start to point
        ax.plot([x_start, row['resilience_mean']], 
               [y_start, row['kl_mean']],
               color=color, alpha=0.3, linewidth=1.5, zorder=1)
        
        # Plot endpoint with marker
        ax.scatter(
            row['resilience_mean'],
            row['kl_mean'],
            marker=marker,
    ...
    # Mark origin
    ax.scatter([0], [0], marker='x', s=200, color='black', linewidths=3, zorder=4, label='Origin')
    ...
    save_path_arrow = output_dir / 'cross_model_kl_vs_resilience_arrows.png'
    plt.savefig(save_path_arrow, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved arrow chart: {save_path_arrow}")
    
    # Create alternative visualization: Grouped horizontal bars
    # Categories on y-axis, with bars extending in 2D space
    print("  Creating grouped bar chart...")
    
    # For this we'll create a figure where each category gets a vertical position,
    # and for each model within that category, we show resilience and KL as connected bars
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 10), sharey=True)
    
    y_positions = {cat: i for i, cat in enumerate(categories)}
    bar_height = 0.15
    model_offsets = {model: i * bar_height for i, model in enumerate(model_names[:4])}
    
    # Left panel: Resilience
    for _, row in df_stats.iterrows():
        model = row['model']
        category = row['category']
        ...
        y_pos = y_positions[category] + model_offsets[model] - (len(model_offsets) - 1) * bar_height / 2
        color = cat_to_color[category]
        
        ax1.barh(y_pos, row['resilience_mean'], height=bar_height*0.9,
                xerr=row['resilience_se'], color=color, alpha=0.7,
    ...
    ax1.set_xlabel('Average resilience score', fontsize=16)
    ax1.set_title('Resilience', fontsize=18, fontweight='bold')
    ax1.grid(True, axis='x', alpha=0.2)
    ax1.set_axisbelow(True)
    
    # Right panel: Counterfactual++ importance
    for _, row in df_stats.iterrows():
        model = row['model']
        category = row['category']
        if model not in model_offsets:
        ...
        ax2.barh(y_pos, row['kl_mean'], height=bar_height*0.9,
                xerr=row['kl_se'], color=color, alpha=0.7,
                edgecolor='white', linewidth=1.5, capsize=3)
    
    ax2.set_xlabel('Counterfactual++ importance', fontsize=16)
    ax2.set_title('Counterfactual++', fontsize=18, fontweight='bold')
    ax2.grid(True, axis='x', alpha=0.2)
    ax2.set_axisbelow(True)
    
    # Model legend (using shapes)
    model_handles_bar = []
    for model in model_names[:4]:
        if model in df_stats['model'].values:
            # Use color patches for the bar chart since shapes don't make sense here
            from matplotlib.patches import Patch
            # We'll use different shades or patterns, but for now just use a gray with label
            model_handles_bar.append(Patch(facecolor='gray', alpha=0.7, label=_legend_model_name(model)))
    
    fig.legend(handles=model_handles_bar, title='Model',
              loc='upper center', bbox_to_anchor=(0.5, -0.03),
              ncol=4, frameon=True, framealpha=0.95,
              fontsize=13, title_fontsize=14)
    
    plt.suptitle('Side-by-side comparison: Resilience and Counterfactual++ by category',
                fontsize=20, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0.05, 1, 0.96])
    
    save_path_bars = output_dir / 'cross_model_kl_vs_resilience_sidebyside_bars.png'
    plt.savefig(save_path_bars, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved side-by-side bar chart: {save_path_bars}")
    
    # Create alternative: Heatmap-style visualization
    print("  Creating heatmap with dual encoding...")
    
    # Pivot data for heatmap
    pivot_resilience = pd.pivot_table(df_stats, values='resilience_mean', 
                                      index='category', columns='model')
    pivot_kl = pd.pivot_table(df_stats, values='kl_mean',
                              index='category', columns='model')
    
    # Reorder columns to match model order
    available_models = [m for m in model_names[:4] if m in pivot_resilience.columns]
    pivot_resilience = pivot_resilience[available_models]
    pivot_kl = pivot_kl[available_models]
    
    # Create figure with heatmap
    fig, ax = plt.subplots(figsize=(12, 8))
    
    # Use resilience as color intensity
    im = ax.imshow(pivot_resilience.values, cmap='YlOrRd', aspect='auto', alpha=0.8)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax, pad=0.02)
    cbar.set_label('Resilience score', fontsize=14, rotation=270, labelpad=25)
    
    # Annotate each cell with KL value
    for i, category in enumerate(pivot_resilience.index):
        for j, model in enumerate(available_models):
            kl_val = pivot_kl.loc[category, model]
            resilience_val = pivot_resilience.loc[category, model]
            
            # Determine text color based on background intensity
            if not np.isnan(resilience_val):
                text_color = 'white' if resilience_val > pivot_resilience.values.mean() else 'black'
            else:
                text_color = 'black'
            
            if not np.isnan(kl_val):
                ax.text(j, i, f'{kl_val:.4f}',
                       ha='center', va='center', fontsize=11,
                       color=text_color, fontweight='bold')
    
    # Set ticks
    ax.set_xticks(np.arange(len(available_models)))
    ax.set_yticks(np.arange(len(pivot_resilience.index)))
    ax.set_xticklabels([_legend_model_name(m) for m in available_models], 
                       rotation=45, ha='right', fontsize=12)
    ax.set_yticklabels(pivot_resilience.index, fontsize=12)
    
    ax.set_xlabel('Model', fontsize=16, fontweight='bold')
    ax.set_ylabel('Category', fontsize=16, fontweight='bold')
    ax.set_title('Dual-encoded heatmap\n(color = resilience, numbers = counterfactual++)',
                fontsize=18, fontweight='bold', pad=15)
    
    plt.tight_layout()
    
    save_path_heatmap = output_dir / 'cross_model_kl_vs_resilience_heatmap.png'
    plt.savefig(save_path_heatmap, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved heatmap: {save_path_heatmap}")
    
    # Create alternative: Parallel coordinates plot
    print("  Creating parallel coordinates plot...")
    
    fig, ax = plt.subplots(figsize=(10, 10))
    
    # Normalize both metrics to 0-1 for better visualization
    resilience_min = df_stats['resilience_mean'].min()
    resilience_max = df_stats['resilience_mean'].max()
    kl_min = df_stats['kl_mean'].min()
    kl_max = df_stats['kl_mean'].max()
    ...
    for _, row in df_stats.iterrows():
        ...
        if resilience_max > resilience_min:
            resilience_norm = (row['resilience_mean'] - resilience_min) / (resilience_max - resilience_min)
        else:
            resilience_norm = 0.5
        
        if kl_max > kl_min:
            kl_norm = (row['kl_mean'] - kl_min) / (kl_max - kl_min)
        else:
            kl_norm = 0.5
        
        # Draw line connecting the two axes
        ax.plot([0, 1], [resilience_norm, kl_norm], 
               color=color, alpha=0.5, linewidth=2, zorder=1)
        
        # Add markers at both ends with model shape
        marker = model_markers.get(model, 'o')
        ax.scatter([0], [resilience_norm], marker=marker, s=80,
                  color=color, edgecolors='white', linewidths=1.5, 
                  alpha=0.8, zorder=3)
        ax.scatter([1], [kl_norm], marker=marker, s=80,
                  color=color, edgecolors='white', linewidths=1.5,
                  alpha=0.8, zorder=3)
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(['Resilience\n(normalized)', 'Counterfactual++\n(normalized)'],
                      fontsize=16, fontweight='bold')
    ax.set_ylabel('Normalized value (0-1)', fontsize=14)
    
    # Add actual value ranges as text
    ax.text(0, -0.12, f'[{resilience_min:.1f} - {resilience_max:.1f}]',
           ha='center', va='top', fontsize=11, transform=ax.transData)
    ax.text(1, -0.12, f'[{kl_min:.4f} - {kl_max:.4f}]',
           ha='center', va='top', fontsize=11, transform=ax.transData)
    
    ax.set_title('Parallel coordinates: Resilience and Counterfactual++',
                fontsize=20, fontweight='bold', pad=15)
    ax.grid(True, axis='y', alpha=0.2)
    ax.set_axisbelow(True)
    
    # Remove y-axis tick labels (not meaningful for normalized data)
    ax.set_yticklabels([])
    
    # Legends
    cat_handles = [
        Line2D([0], [0], color=cat_to_color[cat], linewidth=3,
               alpha=0.8, label=cat)
        for cat in categories
    ]
    ...
    ax.legend(handles=model_handles, title='Model',
             loc='center left', bbox_to_anchor=(1.05, 0.25),
             frameon=True, framealpha=0.95,
             fontsize=12, title_fontsize=13)
    ...
    save_path_parallel = output_dir / 'cross_model_kl_vs_resilience_parallel.png'
    plt.savefig(save_path_parallel, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved parallel coordinates: {save_path_parallel}")
    
    # Create alternative: Position-based bubble chart
    # X-axis = normalized position in CoT, Y-axis = KL, bubble size = resilience
    print("  Creating position-based bubble chart...")
    
    # Need to add position information to df_stats
    # Re-merge with resilience CSV to get position data
    all_data_with_pos = []
    ...
    for model in model_names:
        ...
        if 'position' not in df_resilience.columns:
            continue
            
        merged = pd.merge(
            df_kl[['scenario_id', 'chunk_idx', 'kl_after_vs_before', 'primary_tag', 'n_after_rollouts']],
            df_resilience[['scenario_id', 'chunk_idx', 'avg_fighting_back_score_semantic', 'position']],
            on=['scenario_id', 'chunk_idx'],
            how='inner'
        )
        ...
        all_data_with_pos.append(merged)
    
    if not all_data_with_pos:
        print("  No position data available for position-based plot")
    else:
        df_pos = pd.concat(all_data_with_pos, ignore_index=True)
        
        # Aggregate by model-category: compute mean position, mean KL, mean resilience
        pos_stats_list = []
        for model in model_names[:4]:
            if model not in df_pos['model'].values:
                continue
            
            model_data = df_pos[df_pos['model'] == model]
            ...
            for category in sorted(model_data['category'].unique()):
                ...
                pos_stats_list.append({
                    'model': model,
                    'category': category,
                    'position_mean': float(cat_data['position'].mean()),
                    'position_se': float(cat_data['position'].std() / np.sqrt(len(cat_data))) if len(cat_data) > 1 else 0.0,
                    'kl_mean': float(cat_data['kl_after_vs_before'].mean()),
                    'kl_se': float(cat_data['kl_after_vs_before'].std() / np.sqrt(len(cat_data))) if len(cat_data) > 1 else 0.0,
                    'resilience_mean': float(cat_data['avg_fighting_back_score_semantic'].mean()),
                    'resilience_se': float(cat_data['avg_fighting_back_score_semantic'].std() / np.sqrt(len(cat_data))) if len(cat_data) > 1 else 0.0,
                    'n_sentences': len(cat_data)
                })
        
        df_pos_stats = pd.DataFrame(pos_stats_list)
        
        if df_pos_stats.empty:
            print("  No aggregated position data")
        else:
            # Create figure
            fig, ax = plt.subplots(figsize=(14, 10))
            
            # Normalize resilience to bubble size (50-500 range)
            resilience_min = df_pos_stats['resilience_mean'].min()
            resilience_max = df_pos_stats['resilience_mean'].max()
            
            for _, row in df_pos_stats.iterrows():
                model = row['model']
                category = row['category']
                marker = model_markers.get(model, 'o')
                color = cat_to_color[category]
                
                # Scale resilience to bubble size
                if resilience_max > resilience_min:
                    resilience_norm = (row['resilience_mean'] - resilience_min) / (resilience_max - resilience_min)
                else:
                    resilience_norm = 0.5
                bubble_size = 100 + resilience_norm * 600  # 100-700 range for better visibility
                
                # Plot with position on x-axis, KL on y-axis, size = resilience
                ax.scatter(
                    row['position_mean'],
                    row['kl_mean'],
                    s=bubble_size,
        ...
        ax.set_xlabel('Normalized position in CoT (0 = start, 1 = end)', fontsize=18)
        ax.set_ylabel('Counterfactual++ importance', fontsize=18)
        ax.set_title('Counterfactual++ by CoT position\n(bubble size = resilience score)',
                    fontsize=22, pad=20, fontweight='bold')
        ax.grid(True, alpha=0.2, zorder=1)
        ax.set_axisbelow(True)
        ...
        model_handles = []
        for model in model_names[:4]:
            if model in df_pos['model'].values:
                marker = model_markers.get(model, 'o')
                model_handles.append(
                    Line2D([0], [0], marker=marker, linestyle='None',
                           markerfacecolor='gray', markeredgecolor='white',
        ...
        # Size legend
        size_vals = [resilience_min, (resilience_min + resilience_max)/2, resilience_max]
        size_handles = []
        for val in size_vals:
            if resilience_max > resilience_min:
                norm = (val - resilience_min) / (resilience_max - resilience_min)
            else:
                norm = 0.5
            size = 50 + norm * 450
            size_handles.append(
                Line2D([0], [0], marker='o', linestyle='None',
                       markerfacecolor='gray', markeredgecolor='white',
                       markersize=np.sqrt(size/10), markeredgewidth=1.5,
                       label=f'{val:.1f}', alpha=0.6)
            )
        ...
        ax.legend(handles=size_handles, title='Resilience (bubble size)',
                 loc='upper center', bbox_to_anchor=(0.5, -0.12),
                 ncol=3, frameon=True, framealpha=0.95, edgecolor='gray',
                 fontsize=12, title_fontsize=13)
        ...
        save_path_position = output_dir / 'cross_model_kl_vs_position_bubble.png'
        plt.savefig(save_path_position, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"  Saved position-based bubble chart: {save_path_position}")

def generate_cross_model_entropy_vs_resilience_scatter(model_names: List[str], output_dir: Path) -> None:
    """Create a 2x2 figure: for each model, scatter sentences with
    y = semantic entropy (from chunks_labeled.json) vs x = resilience
    (avg_fighting_back_score_semantic preferred; fallback to persistence_rate_semantic),
    colored by category.

    Saves to analysis/determination/cross_model_entropy_vs_resilience.png.
    """
    print("Creating cross-model entropy vs resilience scatter...")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    # Limit to four models for the 2x2 grid
    selected_models = model_names[:4]

    # Helper to robustly parse categories cell from CSV
    def _parse_categories_cell(val) -> List[str]:
        if isinstance(val, list):
            return [str(x) for x in val]
        ...
        if isinstance(val, str):
            ...
            return [str(x) for x in parts] if parts else ([val] if val else [])
        try:
            return [str(val)]
        except Exception:
            return []

    # Build a global category list to ensure consistent colors and a single legend
    global_cats_set = set()
    for model in selected_models:
        ...
        if not csv_path.exists():
            continue
        try:
            df_tmp = pd.read_csv(csv_path, usecols=['categories'])
        except Exception:
            continue
        for v in df_tmp['categories'].dropna().values.tolist():
            for c in _parse_categories_cell(v):
                if not c:
                    continue
                if str(c).strip().lower() in exclude_cats_lower:
                    continue
                global_cats_set.add(_format_category_label(c))
    global_cats = sorted(global_cats_set)
    # Build a consistent palette mapping across subplots
    palette_colors = sns.color_palette('tab10', n_colors=max(3, min(10, len(global_cats)))) if len(global_cats) <= 10 else sns.color_palette('tab20', n_colors=len(global_cats))
    cat_to_color = {cat: palette_colors[i % len(palette_colors)] for i, cat in enumerate(global_cats)}
    used_cats_set = set()
    ...
    for i in range(4):
        ...
        if not csv_path.exists():
            ax.text(0.5, 0.5, f"No CSV for {model}", transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue
        ...
        try:
            ...
            ax.text(0.5, 0.5, f"Failed to read CSV: {e}", transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue

        if df.empty or 'scenario_id' not in df.columns or 'chunk_idx' not in df.columns or 'categories' not in df.columns:
            ax.text(0.5, 0.5, 'Missing required columns', transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue
        ...
        resilience = None
        if 'avg_fighting_back_score_semantic' in df.columns:
            resilience = df['avg_fighting_back_score_semantic']
        if (resilience is None or resilience.isna().all()) and 'persistence_rate_semantic' in df.columns:
            resilience = df['persistence_rate_semantic']
        if resilience is None or resilience.isna().all():
            ax.text(0.5, 0.5, 'No resilience data', transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue

        # Build (scenario_id, chunk_idx) -> semantic_entropy mapping
        entropy_map: Dict[Tuple[int, int], float] = {}
        scen_ids = sorted(set(int(s) for s in df['scenario_id'].dropna().unique()))
        for scen_id in scen_ids:
            scen_dir = base_dir / f"scenario_{scen_id}"
            chunks_file = scen_dir / "chunks_labeled.json"
            if not chunks_file.exists():
                continue
            try:
            ...
            for ch in chunks_data or []:
                try:
                    cidx = int(ch.get('chunk_index', ch.get('chunk_idx')))
                except Exception:
                    continue
                ent = ch.get('semantic_entropy', None)
                if ent is None:
                    continue
                try:
                    entropy_map[(scen_id, cidx)] = float(ent)
                except Exception:
                    pass

        if not entropy_map:
            ax.text(0.5, 0.5, 'No semantic entropy found', transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue

        # Prepare plotting dataframe: select needed cols and attach entropy
        sub = df[['scenario_id', 'chunk_idx', 'categories']].copy()
        sub['resilience'] = resilience
        # Attach entropy via mapping
        def _lookup_entropy(row):
            key = (int(row['scenario_id']), int(row['chunk_idx']))
            return entropy_map.get(key, np.nan)
        sub['semantic_entropy'] = sub.apply(_lookup_entropy, axis=1)
        sub = sub.dropna(subset=['resilience', 'semantic_entropy'])
        if sub.empty:
            ax.text(0.5, 0.5, 'No matched entropy/resilience', transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue

        # Expand by categories for coloring; robustly parse list-like strings; omit excluded categories
        rows = []
        for _, r in sub.iterrows():
            cats = _parse_categories_cell(r['categories'])
            if not cats:
                continue
            for c in cats:
                if not c:
                ...
                if str(c).strip().lower() in exclude_cats_lower:
                    continue
                rows.append({
                    'resilience': float(r['resilience']),
                    'semantic_entropy': float(r['semantic_entropy']),
                    'category': _format_category_label(c)
                })
        df_plot = pd.DataFrame(rows)
        if df_plot.empty:
            ax.text(0.5, 0.5, 'No categories to plot', transform=ax.transAxes, ha='center', va='center')
            ax.axis('off')
            continue
        ...
        sns.scatterplot(
            ...
            hue_order=global_cats,
            legend=False
        )
        ax.set_title(f"{_legend_model_name(model)}")
        ax.set_xlabel('Average resilience score')
        ax.set_ylabel('Semantic entropy')
        ax.grid(True, alpha=0.3)
        used_cats_set.update(df_plot['category'].unique().tolist())
    # Global legend at bottom using consistent colors
    try:
        from matplotlib.lines import Line2D
        ordered_used = [c for c in global_cats if c in used_cats_set] if global_cats else sorted(used_cats_set)
        handles = [Line2D([0], [0], marker='o', linestyle='None', markeredgecolor=cat_to_color.get(c, 'gray'),
                           markerfacecolor=cat_to_color.get(c, 'gray'), markersize=8, label=c) for c in ordered_used]
        legend = fig.legend(handles=handles, labels=ordered_used, loc='upper center',
                            bbox_to_anchor=(0.5, -0.01), ncol=min(len(ordered_used), 4),
                            framealpha=0.9, title='Category', handletextpad=0.35,
                            borderpad=0.45, labelspacing=0.45, columnspacing=0.75)
        plt.subplots_adjust(bottom=0.11, hspace=0.33, wspace=0.26)
    except Exception:
        pass

    plt.tight_layout(rect=[0, 0.085, 1, 1])
    save_path = output_dir / 'cross_model_entropy_vs_resilience.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    if args.input_dir:
        ...
        # Check if we should skip per-model analysis
        if args.skip_per_model:
            print("\n" + "="*80)
            print("SKIP-PER-MODEL MODE: Only generating cross-model plots")
            print("="*80 + "\n")
            
            # Check which models have existing CSV files
            for model_name in AVAILABLE_MODELS:
                output_dir = Path(f"analysis/{model_name}/determination")
                required_files = [
                    output_dir / "fighting_back_experiments.csv",
                    output_dir / "fighting_back_kl_per_sentence.csv"
                ]
                if all(f.exists() for f in required_files):
                    processed_models.append(model_name)
                    print(f"✓ Found analysis files for {model_name}")
                else:
                    print(f"✗ Missing analysis files for {model_name}, skipping")
        else:
            # Normal mode: run full analysis for each model
            for model_name in AVAILABLE_MODELS:
        ...
        if processed_models:
            ...
            try:
                # Cross-model: per-sentence resilience histograms (2x2, one per model)
                generate_cross_model_sentence_resilience_histograms(processed_models, Path("analysis/determination"))
            except Exception as e:
                print(f"Warning: failed to create cross-model resilience histograms: {e}")
            try:
                # Cross-model: entropy vs resilience (2x2 scatter per model)
                generate_cross_model_entropy_vs_resilience_scatter(processed_models, Path("analysis/determination"))
            except Exception as e:
                print(f"Warning: failed to create entropy vs resilience scatter: {e}")
            try:
                # Cross-model: KL vs resilience scatter (colors by category, shapes by model)
                generate_cross_model_kl_vs_resilience_scatter(processed_models, Path("analysis/determination"))
            except Exception as e:
                print(f"Warning: failed to create KL vs resilience scatter: {e}")
            try:
                # Cross-model: semantic entropy grouped bars by category (ALL categories)
                generate_combined_semantic_entropy_grouped_bars(processed_models, Path("analysis/determination"))
            except Exception as e:
                print(f"Warning: failed to create semantic entropy grouped bars: {e}")
