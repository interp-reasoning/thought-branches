#!/usr/bin/env python3
"""
Realistic test for Nebius API logprob extraction using actual blackmail rollout data.
...
def nebius_get_logprobs(
    ...
    model_id = MODEL_FOLDER_TO_NEBIUS.get(model_folder, MODEL_FOLDER_TO_NEBIUS['qwen3-235b-a22b'])
...
def load_onpolicy_sentence(
    ...
    """
    # Build paths
    base = Path('blackmail_rollouts') / model_folder / f"temperature_{temperature}_top_p_{top_p}"
...
def test_real_examples(min_scored_tokens: int = 4):
    """Test logprob extraction on actual blackmail rollout data."""
    start_time = time.time()
    
    print("\n" + "="*80)
    print("REALISTIC LOGPROB EXTRACTION TESTS")
    print("Using actual blackmail rollout data")
    ...
    # NOTE: Skipping qwq-32b for now since it's not supported by Nebius and we computed its logprobs via the HF model locally
    models = ['qwq-32b', 'qwen3-235b-a22b', 'llama-3_1-nemotron-ultra-253b-v1', 'deepseek-r1-0528'][1:]
    ...
    for model_idx, model_folder in enumerate(models, 1):
        ...
        for ex_idx, (example_type, row) in enumerate(test_examples, 1):
            ...
            try:
                ...
                if api_result['count'] < min_scored_tokens:
                    print(f"\n  ⚠ SKIPPED: Only {api_result['count']} tokens scored (minimum: {min_scored_tokens})")
                    total_tests -= 1  # Don't count this as a test
                    continue
...
def main():
    """Run all tests."""
    parser = argparse.ArgumentParser(description='Test Nebius API logprob extraction on blackmail rollout data')
    parser.add_argument('-mst', '--min-scored-tokens', type=int, default=4, help='Minimum number of scored tokens required (default: 4, skips tests with fewer tokens)')
    args = parser.parse_args()
    
    try:
        test_real_examples(min_scored_tokens=args.min_scored_tokens)
        return 0
    except Exception as e:
        print(f"\n{'='*80}")
        print(f"TEST FAILED")
