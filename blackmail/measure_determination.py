...
parser.add_argument('-rrt', '--rerun_threshold', type=float, default=None, help='Re-run scenario:chunk:strategy if absolute blackmail rate change >= this threshold (e.g., 0.1 for 10%% change). None = disable')
...
def _is_metadata_stale(experiment_result: Dict, current_importance: float, current_prev_blackmail_rate: Optional[float], tolerance: float = 1e-6, rate_threshold: Optional[float] = None) -> bool:
    """Check if experiment metadata (importance_score, prev_blackmail_rate) is stale compared to current chunks_labeled.json.
    
    Args:
        experiment_result: Existing experiment result
        current_importance: Current importance score from chunks_labeled.json
        current_prev_blackmail_rate: Current prev_blackmail_rate from chunks_labeled.json
        tolerance: Tolerance for importance_score comparison (default 1e-6)
        rate_threshold: If set, use this threshold for prev_blackmail_rate comparison instead of tolerance
    ...
    try:
        ...
        stored_prev_rate = experiment_result.get("prev_blackmail_rate")
        ...
        # Check prev_blackmail_rate (use rate_threshold if provided, else tolerance)
        ...
        if stored_prev_rate is not None and current_prev_blackmail_rate is not None:
            change = abs(float(stored_prev_rate) - float(current_prev_blackmail_rate))
            if rate_threshold is not None:
                ...
                if change > rate_tolerance:
                    return True
        elif (stored_prev_rate is None) != (current_prev_blackmail_rate is None):
            # One is None, other is not - metadata changed
            return True
...
def _update_experiment_metadata(experiment_result: Dict, current_importance: float, current_prev_blackmail_rate: Optional[float]) -> bool:
    ...
    try:
        ...
        if experiment_result.get("prev_blackmail_rate") != current_prev_blackmail_rate:
            experiment_result["prev_blackmail_rate"] = current_prev_blackmail_rate
            updated = True
...
def _should_rerun_strategy_by_effect(strategy_blackmail_rate: Optional[float], baseline_prev_blackmail_rate: Optional[float], threshold: Optional[float]) -> bool:
    """Check if a strategy should be re-run based on the absolute effect size.
    
    Args:
        strategy_blackmail_rate: Blackmail rate after applying this strategy
        baseline_prev_blackmail_rate: Baseline blackmail rate (before disruption)
    ...
    try:
        if strategy_blackmail_rate is None or baseline_prev_blackmail_rate is None:
            return False  # Can't compute effect without both values
        
        absolute_effect = abs(float(strategy_blackmail_rate) - float(baseline_prev_blackmail_rate))
    ...
    for exp in results.get("experiments", []):
        ...
        for chunk_data in high_leverage_chunks:
            if int(chunk_data["chunk_idx"]) == chunk_idx:
                ...
                if _update_experiment_metadata(exp, importance_score, prev_blackmail_rate):
                    updated_count += 1
                break
...
def _get_chosen_disruption_cache_path(model_name: str, experiment_type: str) -> Path:
    ...
    cache_dir = Path("blackmail_rollouts") / model_name / ".cache"
...
def _print_initial_similar_sentences() -> None:
    ...
    if args.experiment not in ["chain_disruption_same_model", "chain_disruption_cross_model", "all"]:
        return
    try:
        strategies = _get_disruption_strategies()
        
        # Print for same_model if applicable
        if args.experiment in ["chain_disruption_same_model", "all"]:
            exp_type = 'same_model'
            pool_desc = f"same-model pool ({args.model})"
            ...
            print(f"\n=== Initial most-similar picks per category for chain_disruption_same_model using {pool_desc} ===")
        ...
        # Print for cross_model if applicable
        if args.experiment in ["chain_disruption_cross_model", "all"]:
            exp_type = 'cross_model'
            pool_desc = f"cross-model pool (excluding {args.model})"
            ...
            print(f"\n=== Initial most-similar picks per category for chain_disruption_cross_model using {pool_desc} ===")
            if not chosen_disruptions:
                print("  No chosen disruptions available; nothing to print")
            else:
                threshold = args.similarity_threshold
...
def _build_yes_base_chunk0_dirs(model_name: str) -> List[Path]:
    try:
        base = Path("blackmail_rollouts") / model_name / f"temperature_{str(args.temperature)}_top_p_{str(args.top_p)}" / "yes_base_solution"
        if not base.exists():
            return []
        # scenario_*/chunk_0
        scenario_dirs = [d for d in base.iterdir() if d.is_dir() and d.name.startswith("scenario_")]
        # Sort by numeric scenario index for determinism
        try:
...
def _extract_candidate_sentences_from_rollout(entry: Dict) -> List[str]:
    """Extract all candidate sentences (chunks) from a rollout entry.

    Priority:
    1) Use 'rollout' text if present (already the generated text only)
    2) Else, split 'full_cot' after the Assistant header and use all chunks
    3) Include 'chunk_resampled' if present (deduped with others)
    Filters out markup/tool lines and empties.
    """
    candidates: List[str] = []
    try:
        rollout_text = entry.get("rollout") or ""
        if rollout_text:
            chunks = split_solution_into_chunks(rollout_text)
            for c in chunks or []:
                t = (c or "").strip()
                if not t:
                    continue
            ...
            chunks = split_solution_into_chunks(after)
            for c in chunks or []:
                t = (c or "").strip()
                if not t:
                    continue
...
def _load_candidate_sentences_for_model(model_name: str) -> List[str]:
    ...
    sentences: List[str] = []
    for cdir in _build_yes_base_chunk0_dirs(model_name):
        solutions_file = cdir / "solutions.json"
...
def _get_candidate_pool_with_embeddings(model_name: str) -> Tuple[List[str], np.ndarray]:
    ...
    if not sentences:
        _CANDIDATE_EMBEDDINGS_CACHE[emb_key] = np.zeros((0,))  # placeholder
        return sentences, _CANDIDATE_EMBEDDINGS_CACHE[emb_key]
    try:
        # Prefer batched, numpy output; fallback to CPU if GPU OOM/issue
        try:
            embs = embedding_model.encode(
                sentences,
                batch_size=min(128, max(8, len(sentences))),
            ...
            embs = embedding_model.encode(
                sentences,
                batch_size=min(64, max(8, len(sentences))),
                convert_to_numpy=True,
                show_progress_bar=False
            )
        embs = np.array(embs)
...
def _get_cross_model_candidate_pool_with_embeddings(exclude_model: str) -> Tuple[List[str], np.ndarray]:
    ...
    all_models = _list_models_under_rollouts(Path("blackmail_rollouts"))
...
def _pick_alternative_model(exclude_model: str) -> Optional[str]:
    models = _list_models_under_rollouts(Path("blackmail_rollouts"))
...
async def wait_injection_experiment(
    ...
    for chunk_data in high_leverage_chunks:
        ...
        if chunk_idx in existing_experiments and not args.force:
            ...
            # Check if metadata is stale (importance_score or prev_blackmail_rate changed)
            prev_blackmail_rate = get_prev_blackmail_rate(labeled_chunks, chunk_idx) if labeled_chunks else None
            if _is_metadata_stale(experiment_result, importance_score, prev_blackmail_rate, rate_threshold=args.rerun_threshold):
                print(f"  Chunk {chunk_idx}: Metadata changed (importance or prev_blackmail_rate), discarding existing results")
                    ...
                    if chunk_idx not in existing_experiments:
                        ...
                        for i, exp in enumerate(results["experiments"]):
                            if exp["chunk_idx"] == chunk_idx:
                                results["experiments"][i] = experiment_result
                                break
                    save_results(results, output_dir / "think_injection_results.json")
...
async def chain_disruption_cross_model_experiment(
    ...
    disruption_strategies = _get_disruption_strategies()
    ...
    for chunk_data in high_leverage_chunks:
        ...
        for strategy_name, disruption_text in disruption_strategies:
            ...
            if strategy_name in chosen_disruptions:
                ...
                chosen_text, sim = "", -1.0
            if (not chosen_text) or (sim < args.similarity_threshold):
                print(f"    {strategy_name}: Skipping (similarity {sim:.2f} < threshold {args.similarity_threshold})")
                ...
                if chunk_idx not in existing_experiments:
                    ...
                    for i, exp in enumerate(results["experiments"]):
                        if exp["chunk_idx"] == chunk_idx:
                            results["experiments"][i] = experiment_result
                            break
                save_results(results, output_dir / "chain_disruption_cross_model_results.json")
                continue
            ...
            if rollouts_needed > 0:
                ...
                for completed in asyncio.as_completed(tasks):
                    ...
                    if rollout_result and ("error" not in rollout_result):
                        strategy_results.append(rollout_result)
                        blackmail_count = sum(1 for r in strategy_results if r["contains_blackmail"]) if strategy_results else 0
                        blackmail_rate = (blackmail_count / len(strategy_results)) if strategy_results else 0.0
                        strategy_data = {
                            "strategy": strategy_name,
                            "original_disruption_text": disruption_text,
                            "chosen_text": chosen_text,
                ...
                if chunk_idx not in existing_experiments:
                    ...
                    for i, exp in enumerate(results["experiments"]):
                        if exp["chunk_idx"] == chunk_idx:
                            results["experiments"][i] = experiment_result
                            break
                save_results(results, output_dir / "chain_disruption_same_model_results.json")
                continue
        ...
        if args.rerun_threshold is not None:
            ...
            for sub_name, sub_data in existing_substitutions.items():
                ...
                if sub_name == "self_preservation":
                    continue
                substitution_rate = sub_data.get("blackmail_rate")
...
async def process_scenario(scenario_dir: Path, output_dir: Path = None) -> None:
    ...
    if args.experiment in ['think_injection', 'all']:
        ...
        save_results(think_results, save_dir / "think_injection_results.json")
    ...
    if args.experiment in ['chain_disruption', 'chain_disruption_cross_model', 'chain_disruption_same_model', 'all']:
        ...
        if args.experiment == 'all':
            ...
            if args.experiment == 'chain_disruption':
                ...
                save_results(disruption_results, save_dir / "chain_disruption_cross_model_results.json")
                ...
                save_results(disruption_results, save_dir / "chain_disruption_same_model_results.json")
