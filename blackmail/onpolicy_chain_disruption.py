...
def identify_high_leverage_chunks(labeled_chunks: List[Dict], importance_key: str, top_n: int = 5, random_selection: bool = False) -> List[Dict]:
    ...
    # Random selection path: choose k pre-</think> chunks ignoring importance, excluding non-reasoning buckets
    if random_selection:
...
def process_scenario(
    ...
    *,
    ...
    overwrite_result_json: bool = False,
    ...
    high_leverage = identify_high_leverage_chunks(labeled_chunks, importance_metric, top_n_chunks, random_selection=random_selection)
    ...
    # Results path for this disruption (-dn)
    ...
    if overwrite_result_json:
        print(f"  Overwriting results JSON for target '{target_name}'", flush=True)
    ...
    async def _classify_async(txt: str) -> Tuple[bool, str, Dict[str, Any]]:
        """Classify blackmail behavior - matches measure_determination.py exactly.
            ...
            if classifier_reasoning and "<answer>" in classifier_reasoning:
                ...
                if answer_match:
                    ...
                    if final_verdict != extracted_answer:
                        return extracted_answer, classifier_reasoning, multiclass or {}
            
            return final_verdict, classifier_reasoning, multiclass or {}
        except Exception as e:
        ...
        async def _reclass_driver() -> List[Tuple[int, Dict]]:
            ...
            async def _task(i: int, p: Dict) -> Tuple[int, Dict]:
                async with sem:
                    ...
                    try:
                        contains_blackmail, reasoning, multiclass_data = await _classify_async(txt_full)
                        fixed = dict(p)
                        fixed["contains_blackmail"] = bool(contains_blackmail)
                        ...
                        if isinstance(multiclass_data, dict):
                            for k, v in multiclass_data.items():
                                fixed[k] = v
                        # Remove error marker if classification succeeded
                        if "error" in fixed:
                            del fixed["error"]
                    except Exception as _:
                        fixed = dict(p)
                        fixed["error"] = "classification error: async-failure"
                    return i, fixed
            ...
            async def _task(idx: int, rr: Dict) -> Tuple[int, Dict]:
                async with sem:
                    ...
                    try:
                        contains_blackmail, reasoning, multiclass_data = await _classify_async(txt_full)
                        out = dict(rr)
                        out["contains_blackmail"] = bool(contains_blackmail)
                        ...
                        out = dict(rr)
                        out["error"] = "classification error: async-failure"
                    return idx, out
    ...
    # Reclassify-only mode: reclassify all existing rollouts and exit
    if globals().get('args', type('obj', (), {})()).__dict__.get('reclassify', False):
        print(f"  Reclassify mode enabled: processing existing experiments only")
        reclassified_any = False
        for exp in results.get("experiments", []):
            ch_idx = int(exp.get("chunk_idx", -1))
            if ch_idx < 0:
                continue
            fulls = exp.get("full_rollouts", [])
            if not fulls:
                print(f"  Chunk {ch_idx}: no rollouts to reclassify, skipping")
                continue
            
            print(f"  Chunk {ch_idx}: reclassifying {len(fulls)} existing rollouts...")
            
            # Build prefill for this experiment
            system_prompt = scenario.get("system_prompt", "")
            user_prompt = scenario.get("user_prompt", "")
            email_content = scenario.get("email_content", "")
            selected_resampled_chunk = exp.get("selected_resampled_chunk") or exp.get("selected_next_sentence") or ""
            selected_continuation_full_text = exp.get("selected_continuation_full_text") if include_similar_sentence else None
            if include_similar_sentence and not selected_continuation_full_text:
                selected_continuation_full_text = exp.get("selected_next_sentence")
            ...
            prefix_text_cls = build_resampled_prefix_text(
                ...
                include_continuation=(include_similar_sentence and bool(selected_continuation_full_text)),
            ) if selected_resampled_chunk else "".join(orig_chunks[:ch_idx])
            prefill_cls = build_prefill_text(system_prompt, user_prompt, email_content, prefix_text_cls)
            
            # Reclassify all rollouts
            async def _reclassify_all() -> List[Dict]:
                try:
                    max_conc = int(globals().get('args', type('A', (), {})()).__dict__.get('max_concurrent_reclassifications', 100))
                except Exception:
                    max_conc = 100
                ...
                async def _task(ridx: int, rr: Dict) -> Tuple[int, Dict]:
                    async with sem:
                        txt_local = rr.get("generated_text", "")
                        full_cot = prefill_cls + txt_local
                        try:
                            contains_blackmail, reasoning, multiclass_data = await _classify_async(full_cot)
                            rr_new = dict(rr)
                            rr_new["contains_blackmail"] = bool(contains_blackmail)
                            rr_new["reasoning"] = reasoning
                            rr_new["full_cot"] = full_cot
                            if isinstance(multiclass_data, dict):
                                for k, v in multiclass_data.items():
                                    rr_new[k] = v
                            # Remove error marker if present
                            if "error" in rr_new:
                                del rr_new["error"]
                            return ridx, rr_new
                        except Exception as e:
                            rr_new = dict(rr)
                            rr_new["error"] = f"reclassification error: {type(e).__name__}: {e}"
                            return ridx, rr_new
                
                tasks = [asyncio.create_task(_task(i, rr)) for i, rr in enumerate(fulls)]
                results_local: List[Tuple[int, Dict]] = []
                for f in asyncio.as_completed(tasks):
                    results_local.append(await f)
                results_local.sort(key=lambda x: x[0])
                return [x[1] for x in results_local]
            
            try:
                loop = asyncio.get_event_loop()
                new_classified = loop.run_until_complete(_reclassify_all())
            except RuntimeError:
                new_classified = asyncio.run(_reclassify_all())
            
            # Update experiment with new classifications
            exp["full_rollouts_classified"] = new_classified
            save_results_dedup(results, scenario_out_dir / f"onpolicy_chain_disruption_{target_name}.json")
            print(f"  Chunk {ch_idx}: reclassified {len(new_classified)} rollouts")
            reclassified_any = True
        
        if reclassified_any:
            print(f"  Reclassification complete for target '{target_name}'")
        else:
            print(f"  No experiments found to reclassify for target '{target_name}'")
        return  # Exit early, don't do resampling/generation
    ...
    for idx_counter, ch in enumerate(high_leverage):
        ...
        if (exp_result.success and exp_result.selected_resampled_chunk) or (prior and prior.get("success")):
            ...
            async def _classify_one(txt: str) -> Tuple[bool, str, Dict[str, Any]]:
                """Classify blackmail behavior - matches measure_determination.py exactly.
            ...
            for rr in fulls:
                ...
                try:
                    contains_blackmail, reasoning, multiclass_data = asyncio.get_event_loop().run_until_complete(_classify_one(prefill_cls + txt))
                except RuntimeError:
                    async def _runner():
                        return await _classify_one(prefill_cls + txt)
                    contains_blackmail, reasoning, multiclass_data = asyncio.run(_runner())
                rr_cls = dict(rr)
                rr_cls["contains_blackmail"] = bool(contains_blackmail)
                rr_cls["reasoning"] = reasoning
            ...
            if prior_cls_list:
                ...
                for p in prior_cls_list:
                    ...
                    txt_full = p.get("full_cot") or (prefill_cls + (p.get("generated_text", "") or ""))
                    try:
                        contains_blackmail_fix, reasoning_fix, multiclass_fix = asyncio.get_event_loop().run_until_complete(_classify_one(txt_full))
                    except RuntimeError:
                        async def _runner2():
                            return await _classify_one(txt_full)
                        contains_blackmail_fix, reasoning_fix, multiclass_fix = asyncio.run(_runner2())
                    fixed = dict(p)
                    fixed["contains_blackmail"] = bool(contains_blackmail_fix)
                    fixed["reasoning"] = reasoning_fix
                    fixed["full_cot"] = txt_full
                    # Add multiclass fields
                    if isinstance(multiclass_fix, dict):
                        for k, v in multiclass_fix.items():
                            fixed[k] = v
                    # Remove error marker if classification succeeded
                    if "error" in fixed:
                        del fixed["error"]
...
def main() -> None:
    ...
    parser.add_argument("--overwrite_target_json", action="store_true", default=False, help="Create a fresh results JSON for the specified disruption (-dn) only; clears prior experiments for that target in each scenario")
    parser.add_argument("-reclf", "--reclassify", action="store_true", default=False, help="Reclassify all existing rollouts without generating new ones (skips resampling and rollout generation)")
    ...
    # Random chunk selection flag
    parser.add_argument("-rc", "--random_chunks", action="store_true", default=False, help="Randomly select --top_n_chunks pre-</think> chunks (ignore importance); outputs saved in *_random folder")
    ...
    if args.output_dir is None:
        ...
        if args.random_chunks and not str(output_dir.name).endswith("_random"):
            output_dir = output_dir.parent / f"{output_dir.name}_random"
    ...
    # Load once and reuse across scenarios (skip if only reclassifying)
    if args.reclassify:
        print("Reclassify mode: skipping model and embedder loading")
    ...
    # Store args in globals for access within process_scenario
    globals()['args'] = args
    ...
    for sdir in scenario_dirs:
        for _dn in disruption_names_to_run:
            process_scenario(
                ...
                overwrite_result_json=bool(args.overwrite_target_json),
                ...
                random_selection=bool(args.random_chunks),
            )
