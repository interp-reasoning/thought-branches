...
- Change in whistleblowing rate (after − before)
- Optional severity deltas if category rates are available (whistleblowing repo is binary-only; usually absent)
...
# Severity mapping (not used for whistleblowing repo which is binary-only). Kept for optional backward compatibility.
...
def find_model_name_from_path(p: Path) -> str:
    ...
    try:
        # Expected structure: whistleblower_rollouts/<model>/temperature_.../yes_base_solution_onpolicy_chain_disruption
        i = parts.index('whistleblower_rollouts')
        return parts[i + 1]
    except Exception:
...
def load_cfpp_importance_map(baseline_scenario_dir: Path) -> Dict[int, float]:
    ...
    try:
        ...
        pass
    # Fallback: if no CF++ keys present at all, try to fall back to base importance map
    try:
        if not cfpp_by_chunk:
            base_imp = load_importance_map(baseline_scenario_dir)
            return base_imp
    except Exception:
        pass
    return cfpp_by_chunk
...
def compute_prev_metrics(baseline_scenario_dir: Path, chunk_idx: int) -> Tuple[Optional[float], Dict[str, float], int]:
    """Compute baseline whistleblowing rate and category distribution from solutions.json if present."""
    ...
    # Whistleblowing rate using only solutions with an explicit contains_whistleblowing classification
    bm_vals: List[int] = []
    try:
        classified_solutions = [s for s in sols if isinstance(s, dict) and s.get('contains_whistleblowing') is not None]
        bm_vals = [1 if bool(s.get('contains_whistleblowing')) else 0 for s in classified_solutions]
...
def compute_after_metrics(exp: Dict) -> Tuple[Optional[float], Dict[str, float], int]:
    """Compute after whistleblowing rate and category distribution from full_rollouts_classified."""
    ...
    for r in cls:
        if not isinstance(r, dict):
            continue
        cb = r.get('contains_whistleblowing', None)
        if cb is not None:
...
def _prepare_base_solutions_cache(solutions_path: Path, embedder, batch_size: int = 128) -> List[Dict[str, Any]]:
    ...
    Returns a list of dicts with keys: sentences, embeddings (np.ndarray), contains_whistleblowing, primary_category
    """
    ...
    for s in tqdm(sols, desc=f"Embedding sentences from {solutions_path.parent.name}", leave=False):
        ...
        cb = s.get('contains_whistleblowing', None)
        if cb is not None:
            cb = bool(cb)
        cache.append({
            'sentences': sents,
            'embeddings': embs,
            'contains_whistleblowing': cb,
...
def _compute_base_after_metrics(
    ...
    if not included:
        return None, {}, 0
    cb_vals = [1 if s.get('contains_whistleblowing') else 0 for s in included if s.get('contains_whistleblowing') is not None]
...
def analyze_onpolicy_base(input_dir: Path, output_dir: Path, targets: Optional[List[str]] = None, max_scenarios: Optional[int] = None, include_embedding_analyses: bool = True) -> None:
        ...
        for res_path in tqdm(files, desc=f"{sdir.name}: targets", leave=False):
            ...
            for exp in tqdm(exps, desc=f"{res_path.name}: chunks", leave=False):
                ...
                records.append({
                    ...
                    'whistleblowing_rate_before': prev_rate,
                    'whistleblowing_rate_after': after_rate,
                    'delta_whistleblowing_rate': delta_rate,
                    ...
                    'whistleblowing_rate_after_base': base_after_rate,
                    'delta_whistleblowing_rate_base': delta_rate_base,
                    ...
                    'whistleblowing_rate_after_base_importance': base_imp_after_rate,
                    'delta_whistleblowing_rate_base_importance': delta_rate_base_imp,
    ...
    for tgt, sdf in df.groupby(group_cols):
        ...
        valid_rate = sdf.dropna(subset=['whistleblowing_rate_after'])
        valid_sev = sdf.dropna(subset=['severity_after'])
        # deltas where both before and after present
        valid_delta_rate = sdf.dropna(subset=['delta_whistleblowing_rate'])
        ...
        after_rate_mean, after_rate_se, n_after_rate = mean_se(valid_rate['whistleblowing_rate_after'])
        before_rate_mean, before_rate_se, n_before_rate = mean_se(valid_rate['whistleblowing_rate_before'])  # note: subset matches valid_rate
        # Principled delta SE via binomial propagation with inverse-variance weighting across chunks
        valid_w = sdf.dropna(subset=['whistleblowing_rate_after', 'whistleblowing_rate_before', 'num_rollouts', 'num_prev_rollouts'])
        deltas: List[float] = []
        weights: List[float] = []
        for _idx, row in valid_w.iterrows():
            try:
                p_after = float(row['whistleblowing_rate_after'])
                p_before = float(row['whistleblowing_rate_before'])
        ...
        if weights:
            ...
            delta_rate_mean, delta_rate_se, n_delta_rate = mean_se(valid_delta_rate['delta_whistleblowing_rate'])
        ...
        valid_rate_base = sdf.dropna(subset=['whistleblowing_rate_after_base']) if 'whistleblowing_rate_after_base' in sdf.columns else sdf.iloc[0:0]
        valid_rate_base_imp = sdf.dropna(subset=['whistleblowing_rate_after_base_importance']) if 'whistleblowing_rate_after_base_importance' in sdf.columns else sdf.iloc[0:0]
        valid_delta_rate_base = sdf.dropna(subset=['delta_whistleblowing_rate_base']) if 'delta_whistleblowing_rate_base' in sdf.columns else sdf.iloc[0:0]
        valid_delta_rate_base_imp = sdf.dropna(subset=['delta_whistleblowing_rate_base_importance']) if 'delta_whistleblowing_rate_base_importance' in sdf.columns else sdf.iloc[0:0]
        ...
        after_rate_base_mean, after_rate_base_se, n_after_rate_base = mean_se(valid_rate_base['whistleblowing_rate_after_base']) if not valid_rate_base.empty else (0.0, 0.0, 0)
        after_rate_bi_mean, after_rate_bi_se, n_after_rate_bi = mean_se(valid_rate_base_imp['whistleblowing_rate_after_base_importance']) if not valid_rate_base_imp.empty else (0.0, 0.0, 0)

        # Weighted delta for base variant using counts
        delta_rate_base_mean, delta_rate_base_se, n_delta_rate_base = (0.0, 0.0, 0)
        if 'whistleblowing_rate_after_base' in sdf.columns and 'num_base_included' in sdf.columns:
            valid_wb = sdf.dropna(subset=['whistleblowing_rate_after_base', 'whistleblowing_rate_before', 'num_base_included', 'num_prev_rollouts'])
            deltas_b = []
            weights_b = []
            for _i, row in valid_wb.iterrows():
                try:
                    p_after_b = float(row['whistleblowing_rate_after_base'])
                    p_before_b = float(row['whistleblowing_rate_before'])
            ...
            if weights_b:
                ...
                delta_rate_base_mean, delta_rate_base_se, n_delta_rate_base = mean_se(valid_delta_rate_base['delta_whistleblowing_rate_base'])

        # Weighted delta for base_importance variant using counts
        delta_rate_bi_mean, delta_rate_bi_se, n_delta_rate_bi = (0.0, 0.0, 0)
        if 'whistleblowing_rate_after_base_importance' in sdf.columns and 'num_base_importance_included' in sdf.columns:
            valid_wbi = sdf.dropna(subset=['whistleblowing_rate_after_base_importance', 'whistleblowing_rate_before', 'num_base_importance_included', 'num_prev_rollouts'])
            deltas_bi = []
            weights_bi = []
            for _i, row in valid_wbi.iterrows():
                try:
                    p_after_bi = float(row['whistleblowing_rate_after_base_importance'])
                    p_before_bi = float(row['whistleblowing_rate_before'])
            ...
            if weights_bi:
                ...
                delta_rate_bi_mean, delta_rate_bi_se, n_delta_rate_bi = mean_se(valid_delta_rate_base_imp['delta_whistleblowing_rate_base_importance'])
        ...
        agg_rows.append({
            ...
            'whistleblowing_rate_after_mean': after_rate_mean,
            'whistleblowing_rate_after_se': after_rate_se,
            'whistleblowing_rate_before_mean': before_rate_mean,
            'whistleblowing_rate_before_se': before_rate_se,
            'delta_whistleblowing_rate_mean': delta_rate_mean,
            'delta_whistleblowing_rate_se': delta_rate_se,
            ...
            'whistleblowing_rate_after_base_mean': after_rate_base_mean,
            'whistleblowing_rate_after_base_se': after_rate_base_se,
            'delta_whistleblowing_rate_base_mean': delta_rate_base_mean,
            'delta_whistleblowing_rate_base_se': delta_rate_base_se,
            ...
            'whistleblowing_rate_after_base_importance_mean': after_rate_bi_mean,
            'whistleblowing_rate_after_base_importance_se': after_rate_bi_se,
            'delta_whistleblowing_rate_base_importance_mean': delta_rate_bi_mean,
            'delta_whistleblowing_rate_base_importance_se': delta_rate_bi_se,
    ...
    # Plots: delta whistleblowing rate and delta severity per target
    if not df_summary.empty:
        plt.figure(figsize=(max(8, 2 + 1.4 * len(df_summary)), 5))
        x = np.arange(len(df_summary))
        vals = df_summary['delta_whistleblowing_rate_mean'].astype(float).values if 'delta_whistleblowing_rate_mean' in df_summary.columns else np.zeros(len(df_summary))
        errs = df_summary['delta_whistleblowing_rate_se'].astype(float).values if 'delta_whistleblowing_rate_se' in df_summary.columns else np.zeros(len(df_summary))
        plt.bar(x, vals, yerr=errs, capsize=4, alpha=0.85)
        plt.axhline(0.0, color='black', linewidth=1)
        plt.xticks(x, [ _format_target_label(t) for t in df_summary['target_name'].tolist() ], rotation=15, ha='right')
        plt.ylabel('Δ Whistleblowing rate (after − before)')
        plt.title('On-policy chain disruption: change in whistleblowing rate by target')
        plt.tight_layout()
        plt.savefig(output_dir / 'onpolicy_delta_whistleblowing_rate_by_target.png', dpi=300, bbox_inches='tight')
        plt.close()

        # (Severity plots removed per whistleblowing repo requirements)
        ...
        # Base: Δ whistleblowing rate by target (embedding-based) – skip if disabled
        if include_embedding_analyses and 'delta_whistleblowing_rate_base_mean' in df_summary.columns:
            plt.figure(figsize=(max(8, 2 + 1.4 * len(df_summary)), 5))
            vals = df_summary['delta_whistleblowing_rate_base_mean'].astype(float).values
            errs = df_summary['delta_whistleblowing_rate_base_se'].astype(float).values if 'delta_whistleblowing_rate_base_se' in df_summary.columns else np.zeros(len(df_summary))
            plt.bar(x, vals, yerr=errs, capsize=4, alpha=0.85)
            plt.axhline(0.0, color='black', linewidth=1)
            plt.xticks(x, [ _format_target_label(t) for t in df_summary['target_name'].tolist() ], rotation=15, ha='right')
            plt.ylabel('Δ Whistleblowing rate (after − before)')
            plt.title('Base: Δ whistleblowing rate by target')
            plt.tight_layout()
            plt.savefig(output_dir / 'base_delta_whistleblowing_rate_by_target.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        # (Severity plots removed)

        # Base-importance: Δ whistleblowing rate by target (embedding-based) – skip if disabled
        if include_embedding_analyses and 'delta_whistleblowing_rate_base_importance_mean' in df_summary.columns:
            plt.figure(figsize=(max(8, 2 + 1.4 * len(df_summary)), 5))
            vals = df_summary['delta_whistleblowing_rate_base_importance_mean'].astype(float).values
            errs = df_summary['delta_whistleblowing_rate_base_importance_se'].astype(float).values if 'delta_whistleblowing_rate_base_importance_se' in df_summary.columns else np.zeros(len(df_summary))
            plt.bar(x, vals, yerr=errs, capsize=4, alpha=0.85)
            plt.axhline(0.0, color='black', linewidth=1)
            plt.xticks(x, [ _format_target_label(t) for t in df_summary['target_name'].tolist() ], rotation=15, ha='right')
            plt.ylabel('Δ Whistleblowing rate (after − before)')
            plt.title('Base-importance: Δ whistleblowing rate by target')
            plt.tight_layout()
            plt.savefig(output_dir / 'base_importance_delta_whistleblowing_rate_by_target.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        try:
            ...
            for sdir in off_scen:
                ...
                for exp in (data_off or {}).get('experiments', []):
                    ...
                    prev_br = exp.get('prev_whistleblowing_rate', None)
                    for dr in exp.get('disruption_results', []) or []:
                        tgt_name = dr.get('strategy') or 'unknown'
                        br_after = dr.get('whistleblowing_rate', None)
                        delta_br = None
                        try:
                        ...
                        off_records.append({
                            ...
                            'chunk_idx': ch_idx,
                            'delta_whistleblowing_rate_off': delta_br,
            ...
            if off_records:
                ...
                for tgt, sdf_off in df_off.groupby('target_name'):
                    ...
                    br_m, br_se, n_br = mean_se_series(sdf_off['delta_whistleblowing_rate_off']) if 'delta_whistleblowing_rate_off' in sdf_off else (0.0, 0.0, 0)
                    ...
                    comp_rows.append({
                        'target_name': tgt,
                        'delta_whistleblowing_rate_off_mean': br_m,
                        'delta_whistleblowing_rate_off_se': br_se,
                ...
                # Comparison: Δ whistleblowing rate by target (after − before)
                plt.figure(figsize=(max(10, 2 + 1.8 * len(df_join)), 6))
                x_idx = np.arange(len(df_join))
                width = 0.4
                on_vals = df_join['delta_whistleblowing_rate_mean'].fillna(0.0).astype(float).values if 'delta_whistleblowing_rate_mean' in df_join.columns else np.zeros(len(df_join))
                on_errs = df_join['delta_whistleblowing_rate_se'].fillna(0.0).astype(float).values if 'delta_whistleblowing_rate_se' in df_join.columns else np.zeros(len(df_join))
                off_vals = df_join['delta_whistleblowing_rate_off_mean'].fillna(0.0).astype(float).values if 'delta_whistleblowing_rate_off_mean' in df_join.columns else np.zeros(len(df_join))
                off_errs = df_join['delta_whistleblowing_rate_off_se'].fillna(0.0).astype(float).values if 'delta_whistleblowing_rate_off_se' in df_join.columns else np.zeros(len(df_join))
                ...
                plt.axhline(0.0, color='black', linewidth=1)
                plt.xticks(x_idx, [ _format_target_label(t) for t in df_join['target_name'].astype(str).tolist() ], rotation=15, ha='right')
                plt.ylabel('Δ Whistleblowing rate (after − before)')
                plt.title('On-policy vs off-policy: Δ whistleblowing rate by sentence continuation')
                plt.legend()
                plt.tight_layout()
                plt.savefig(output_dir / 'onpolicy_vs_offpolicy_delta_whistleblowing_rate_by_target.png', dpi=300, bbox_inches='tight')
                plt.close()
                ...
                try:
                    df_on_chunks = df[['scenario_id', 'chunk_idx', 'target_name', 'delta_whistleblowing_rate', 'delta_severity', 'kl_after_vs_before']].copy()
                    df_off_chunks = df_off[['scenario_id', 'chunk_idx', 'target_name', 'delta_whistleblowing_rate_off', 'delta_severity_off', 'kl_after_vs_before_off']].copy()
                    ...
                    if not df_matched.empty:
                        ...
                        for tgt, sdfm in df_matched.groupby('target_name'):
                            on_dr_m, on_dr_se = _mean_se(sdfm['delta_whistleblowing_rate'])
                            off_dr_m, off_dr_se = _mean_se(sdfm['delta_whistleblowing_rate_off'])
                        ...
                        plt.axhline(0.0, color='black', linewidth=1)
                        plt.xticks(x_m, [ _format_target_label(t) for t in df_m_agg['target_name'].astype(str).tolist() ], rotation=15, ha='right')
                        plt.ylabel('Δ Whistleblowing rate (after − before)')
                        plt.title('Matched only: on-policy vs off-policy Δ whistleblowing rate by target')
                        plt.legend()
                        plt.tight_layout()
                        plt.savefig(output_dir / 'onpolicy_vs_offpolicy_delta_whistleblowing_rate_by_target_matched.png', dpi=300, bbox_inches='tight')
                        plt.close()
...
def _compute_offpolicy_agg_for_model(model: str) -> Optional[pd.DataFrame]:
    try:
        off_dir = Path(f"whistleblower_rollouts/{model}/temperature_0.7_top_p_0.95/yes_base_solution_chain_disruption")
        ...
        for tgt, sdf in df_off.groupby('target_name'):
            def _mean_se(s: pd.Series) -> Tuple[float, float, int]:
                ...
                se = float(vals.std(ddof=1) / np.sqrt(len(vals))) if len(vals) > 1 else 0.0
                return m, se, int(len(vals))
            br_m, br_se, n_br = _mean_se(sdf['delta_whistleblowing_rate_off']) if 'delta_whistleblowing_rate_off' in sdf else (0.0, 0.0, 0)
...
def _format_target_label(t: str) -> str:
    try:
        s = str(t).replace('_', ' ').strip().lower()
        return s[:1].upper() + s[1:] if s else str(t)
    except Exception:
        return str(t)
...
def _plot_grouped_bars_by_target(df: pd.DataFrame, value_col: str, se_col: str, models: List[str], out_path: Path, title: str, y_label: str, ylim: Optional[Tuple[float, float]] = None) -> None:
    ...
    target_labels = [_format_target_label(t) for t in targets]
...
def generate_combined_onpolicy_across_models(model_names: List[str], combined_output_dir: Path, include_embedding_analyses: bool = True) -> None:
    try:
        ...
        ylim_delta_br = _shared_ylim_delta(
            df_all.get('delta_whistleblowing_rate_mean', pd.Series(dtype=float)),
            df_all.get('delta_whistleblowing_rate_se', pd.Series(dtype=float)),
            df_off_all.get('delta_whistleblowing_rate_off_mean', pd.Series(dtype=float)) if not df_off_all.empty else None,
            df_off_all.get('delta_whistleblowing_rate_off_se', pd.Series(dtype=float)) if not df_off_all.empty else None,
        )
        ...
        # (Severity shared y-limits removed)

        # Shared y-limits for base metrics
        ylim_delta_br_base = _shared_ylim_delta(
            df_all.get('delta_whistleblowing_rate_base_mean', pd.Series(dtype=float)) if 'delta_whistleblowing_rate_base_mean' in df_all.columns else pd.Series(dtype=float),
            df_all.get('delta_whistleblowing_rate_base_se', pd.Series(dtype=float)) if 'delta_whistleblowing_rate_base_se' in df_all.columns else pd.Series(dtype=float),
        )
        ...
        # (Severity base shared y-limits removed)

        # Shared y-limits for base_importance metrics
        ylim_delta_br_bi = _shared_ylim_delta(
            df_all.get('delta_whistleblowing_rate_base_importance_mean', pd.Series(dtype=float)) if 'delta_whistleblowing_rate_base_importance_mean' in df_all.columns else pd.Series(dtype=float),
            df_all.get('delta_whistleblowing_rate_base_importance_se', pd.Series(dtype=float)) if 'delta_whistleblowing_rate_base_importance_se' in df_all.columns else pd.Series(dtype=float),
        )
        ...
        # (Severity base-importance shared y-limits removed)

        # On-policy plots (scaled to shared limits)
        _plot_grouped_bars_by_target(
            df_all[['model', 'target_name', 'delta_whistleblowing_rate_mean', 'delta_whistleblowing_rate_se']].copy(),
            'delta_whistleblowing_rate_mean',
            'delta_whistleblowing_rate_se',
            models_order,
            out_base / 'combined_onpolicy_delta_whistleblowing_rate_by_target.png',
            'On-policy: Δ whistleblowing rate by target (all models)',
            'Δ Whistleblowing rate (after − before)',
            ylim=ylim_delta_br,
        )
        ...
        # (Severity combined on-policy plot removed)

        # Base plots across models (embedding-based) – skip if disabled
        if include_embedding_analyses:
            if {'delta_whistleblowing_rate_base_mean', 'delta_whistleblowing_rate_base_se'} <= set(df_all.columns):
                _plot_grouped_bars_by_target(
                    df_all[['model', 'target_name', 'delta_whistleblowing_rate_base_mean', 'delta_whistleblowing_rate_base_se']].copy(),
                    'delta_whistleblowing_rate_base_mean',
                    'delta_whistleblowing_rate_base_se',
                    models_order,
                    out_base / 'combined_base_delta_whistleblowing_rate_by_target.png',
                    'Base: Δ whistleblowing rate by target (all models)',
                    'Δ Whistleblowing rate (after − before)',
            ...
            # (Severity combined base plot removed)

        # Base-importance plots across models (embedding-based) – skip if disabled
        if include_embedding_analyses:
            if {'delta_whistleblowing_rate_base_importance_mean', 'delta_whistleblowing_rate_base_importance_se'} <= set(df_all.columns):
                _plot_grouped_bars_by_target(
                    df_all[['model', 'target_name', 'delta_whistleblowing_rate_base_importance_mean', 'delta_whistleblowing_rate_base_importance_se']].copy(),
                    'delta_whistleblowing_rate_base_importance_mean',
                    'delta_whistleblowing_rate_base_importance_se',
                    models_order,
                    out_base / 'combined_base_importance_delta_whistleblowing_rate_by_target.png',
                    'Base-importance: Δ whistleblowing rate by target (all models)',
                    'Δ Whistleblowing rate (after − before)',
            ...
            # (Severity combined base-importance plot removed)

        # Off-policy aggregates per model
        if not df_off_all.empty:
            _plot_grouped_bars_by_target(
                df_off_all[['model', 'target_name', 'delta_whistleblowing_rate_off_mean', 'delta_whistleblowing_rate_off_se']].copy(),
                'delta_whistleblowing_rate_off_mean',
                'delta_whistleblowing_rate_off_se',
                models_order,
                out_base / 'combined_offpolicy_delta_whistleblowing_rate_by_target.png',
                'Off-policy: Δ whistleblowing rate by target (all models)',
                'Δ Whistleblowing rate (after − before)',
                ylim=ylim_delta_br,
            )
            _plot_grouped_bars_by_target(
            ...
            # (Severity combined off-policy plot removed)
        else:
        ...
        if not df_off_all.empty:
            ...
            _plot_grouped_bars_by_target_dual(
                df_all[['model', 'target_name', 'delta_whistleblowing_rate_mean', 'delta_whistleblowing_rate_se']].copy(),
                df_off_all[['model', 'target_name', 'delta_whistleblowing_rate_off_mean', 'delta_whistleblowing_rate_off_se']].copy(),
                'delta_whistleblowing_rate_mean', 'delta_whistleblowing_rate_se', 'delta_whistleblowing_rate_off_mean', 'delta_whistleblowing_rate_off_se',
                models_order,
                out_base / 'combined_onpolicy_offpolicy_delta_whistleblowing_rate_by_target.png',
                'On-policy vs off-policy: Δ whistleblowing rate by sentence continuation',
                'Δ Whistleblowing rate (after − before)',
                ylim_delta_br,
            )
            _plot_grouped_bars_by_target_dual(
...
def main() -> None:
    ...
    for model in args.models:
        input_dir = Path(f"whistleblower_rollouts/{model}/temperature_0.7_top_p_0.95/yes_base_solution_onpolicy_chain_disruption")
